...
<section class="content">
    <div class="container">
        <div class="books-wrap">
            ...
            <div class="go-to">
                <p><fmt:message key="title.readers"/></p>
                <div class="books-list">
                    <c:forEach var="reader" items="${requestScope.readersPage.objects}">
                        <div class="book-info">
                            <div class="librarians-list">
                                ...
                                <form class="accordion-btns" method="get"
                                    ...
                                    <button type="submit"><fmt:message key="button.show.abonnement"/></button>
                                </form>
                            </div>
                        </div>
                    </c:forEach>
    ...
    <c:if test="${requestScope.readersPage.objects.size() != 0}" >
        <form class="pagination" id="form1" method="get"
              action="${pageContext.request.contextPath}/app/librarian/readers">
            <c:if test="${(requestScope.readersPage.totalPages != 1) && (requestScope.readersPage.currentPage !=0)}">
                <button form="form1" type="submit" name="page" value="0">
                    <fmt:message key="button.first"/>
                </button>
            </c:if>
            <c:if test="${requestScope.readersPage.previousPage != null}">
                <button type="submit" name="page" form="form1"
                        value="${requestScope.readersPage.previousPage}">
                    <fmt:message key="button.left"/></button>
            </c:if>
            <c:if test="${requestScope.readersPage.nextPage != null}">
                <button type="submit" name="page" form="form1"
                        value="${requestScope.readersPage.nextPage}">
                    <fmt:message key="button.right"/>
                </button>
            </c:if>

            <c:if test="${(requestScope.readersPage.totalPages != 1) && (requestScope.readersPage.currentPage !=
                    requestScope.readersPage.lastPage)}">
                <button type="submit" name="page" form="form1" value="${requestScope.readersPage.lastPage}">
